...
namespace ch
{
    ...
    static inline std::uint8_t get_castle_mask(const Board& b)
    {
        std::uint8_t m = 0;
    ...
    static inline void set_castle_mask(Board& b, std::uint8_t m)
    {
        b.set_castle(Color::White, true, (m & (1u << 0)) != 0); // WK
    ...
    static inline bool piece_at(const Board& b, int sq, Color& outC, PieceKind& outK)
    {
        ...
        if (b.occ(Color::White) & mask)
        {
            outC = Color::White;
            for (int k=0; k<6; ++k)
                if (b.bb(outC, PieceKind(k)) & mask) { outK = PieceKind(k); return true; }
        }
        else
        {
            outC = Color::Black;
    ...
    void make_move(Board& b, Move m, State& st)
    {
        ...
        st.fullmove = static_cast<uint16_t>(b.fullmove_number());
        st.promo_code = static_cast<uint8_t>(m.promo_code());
        st.was_ep = false;
        st.was_castle = false;
        ...
        else if (st.moved == PieceKind::King)
        {
            ...
            if (kingSide || queenSide)
            {
                ...
                const bool ks = kingSide;
                const int rf = castle_rook_from(side, ks);
                const int rt = castle_rook_to(side, ks);
